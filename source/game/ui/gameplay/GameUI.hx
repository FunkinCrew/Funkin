...
class GameUI extends FlxTypedGroup<FlxBasic>
{
	...
	public var ratingMap:Map<String, Float> = ['sick' => 1, 'good' => 0.67, 'bad' => 0.34, 'shit' => 0];
	...
	override public function update(elapsed:Float)
	{
		...
		iconP1.scale.set(FlxMath.lerp(1, iconP1.scale.x, CoolUtil.boundTo(1 - (elapsed * 11), 0, 1)),
			FlxMath.lerp(1, iconP1.scale.y, CoolUtil.boundTo(1 - (elapsed * 11), 0, 1)));
		iconP2.scale.set(FlxMath.lerp(1, iconP2.scale.x, CoolUtil.boundTo(1 - (elapsed * 11), 0, 1)),
			FlxMath.lerp(1, iconP2.scale.y, CoolUtil.boundTo(1 - (elapsed * 11), 0, 1)));
	...
	public function updateScoreTxt()
	{
		...
		scoreTxt.text = 'Score: ' + PlayState.songScore + ' - Misses: ' + PlayState.songMisses + ' - Accuracy: [$acc% | $rankStr]';
	}
