...
class TweenV {
	...
	var to:Float;
	var type:TType;
	var plays:Int; // -1 = infinity, 1 et plus = number of executions (1 per default)
	var varType:TVVar; 
...
class SndTV {
	...
	public inline function create(parent:Snd, vartype:TVVar, to:Float, ?tp:TType, ?duration_ms:Float):TweenV {
		return create_(parent, vartype, to, tp, duration_ms);
	}
	
	public function exists(p:Snd):Bool {
		for (t in tlist)
			if (t.parent == p)
				return true;
		return false;
	...
	function create_(p:Snd, vartype:TVVar,to:Float, ?tp:TType, ?duration_ms:Float):TweenV {
		if (duration_ms == null)
		...
		{
			// We delete the previous tweens applied to the same variable
		...
		if (pool.length == 0) {
			t = new TweenV(
				...
				0.0,
				vartype,
				1 / (duration_ms * fps / 1000), // One second
				from,
				to,
				tp,
				1,
		...
		if (t.from == t.to)
			t.ln = 1; // Unnecessary tweening: but we make sure that an update() and an end() will be called
	...
	public static inline function bezier(t:Float, p0:Float, p1:Float, p2:Float, p3:Float):Int {
		return fastPow3(1 - t) * p0 +
		       3 * (t * fastPow2(1 - t) * p1 + fastPow2(t) * (1 - t) * p2) +
		       fastPow3(t) * p3;
	}
	
	// Removal of the tween without any call to the onUpdate, onUpdateT and onEnd (!) callbacks
	public function killWithoutCallbacks(parent:Snd):Bool {
		for (t in tlist.backWardIterator())
			if (t.parent == parent) {
				forceTerminateTween(t);
				return true;
	...
	public function terminateAll() {
		for (t in tlist) t.ln = 1;
		update();
	}
	
	inline function onUpdate(t:TweenV, n:Float) {
		if (t.onUpdate != null) t.onUpdate(t);
	}
	
	inline function onEnd(t:TweenV) {
		if (t.onEnd != null) t.onEnd(t);
	}
