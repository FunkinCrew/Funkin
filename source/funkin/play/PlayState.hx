...
class PlayState extends MusicBeatSubState
{
  ...
  **/
  ...
  {
    var stage = currentChart?.stage;
    return (stage == null || stage == '') ? Constants.DEFAULT_STAGE : stage;
  }
  ...
  {
    if (FlxG.sound.music == null) throw 'Cannot retrieve song length from null song';
  ...
  public function new(?params:PlayStateParams)
  {
    ...
    var params:PlayStateParams = params ?? lastParams ?? throw 'PlayState constructor called with no available parameters.';
    lastParams = params;
  ...
  function buildDiscordRPCState():String
  {
    ...
    {
      trace("WARN: Difficulty data for RPC is null.");
    }
  ...
  function zoomIntoResultsScreen(isNewHighscore:Bool, ?prevScoreData:SaveScoreData):Void
  {
    ...
    // Else, zoom in on GF.
    var boyfriend = currentStage?.getBoyfriend();
    var girlfriend = currentStage?.getGirlfriend();
  ...
  {
    if (currentChart == null) throw 'Cannot move to results screen with a null chart.';
    persistentUpdate = false;
    vocals?.stop();
    camHUD.alpha = 1;
    ...
    // For some reason the null check above seems to be ignored by the compiler so null safety is turned off here
    @:nullSafety(Off)
