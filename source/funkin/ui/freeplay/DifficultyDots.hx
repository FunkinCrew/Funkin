...
class DifficultyDots extends FlxTypedSpriteGroup<DifficultyDot>
{
  ...
  public function regenDots(diffArray:Array<String>):Void
  {
    ...
    // Add forced erected inactive dots (they will be invisible by default)
    erectCheck();
  }

  public function refreshDots(?daSongData:FreeplayState.FreeplaySongData, ?currDiffString:String):Void
  {
    var visibleDots:Array<DifficultyDot> = [];
    for (d in usedDots)
      if (d != null && d.visible) visibleDots.push(d);

    final totalRows:Int = Math.ceil(visibleDots.length / Constants.DEFAULT_FREEPLAY_DOTS_IN_ROW);

    var _row:Int = 0;
    var _col:Float = 0;
    var _curDotSpr:DifficultyDot = null;

    for (i in 0...visibleDots.length)
    {
      _curDotSpr = visibleDots[i];
      ...
      final dotsInCurrentRow:Int = (_row == totalRows - 1) ? (visibleDots.length - _row * Constants.DEFAULT_FREEPLAY_DOTS_IN_ROW) : Constants.DEFAULT_FREEPLAY_DOTS_IN_ROW;
    ...
    for (d in usedDots)
    {
      ...
      if (!d.visible)
      {
        d.updateState(INACTIVE, DESELECTED);
        d.important = false;
      }
    }
