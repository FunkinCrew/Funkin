...
class FreeplayState extends MusicBeatSubState
{
  ...
  override function create():Void
  {
    ...
    letterSort.changeSelectionCallback = (str) -> {
      ...
      else if (grpCapsules.activeSongItems.length > 0)
      {
        curSelected = 1;
        changeSelection();
      }
  ...
  function handleDebugKeys():Void
  {
    ...
    if (controls.DEBUG_STAGE)
    {
      ...
      var targetSongID = grpCapsules.activeSongItems[curSelected]?.freeplayData?.data.id ?? 'unknown';
      if (targetSongID == 'unknown')
      {
        ...
        curSelected = grpCapsules.activeSongItems.indexOf(targetSong);
        changeSelection(0);
        targetSongID = grpCapsules.activeSongItems[curSelected]?.freeplayData?.data.id ?? 'unknown';
      }
  ...
  private function handleTouchCapsuleClick():Void
  {
    ...
    if (TouchUtil.pressAction() && !TouchUtil.overlaps(diffSelRight, funnyCam) && !draggingDifficulty)
    {
      ...
      {
        final capsule = grpCapsules.activeSongItems[i];
  ...
  function handleTouchSelectionScroll(elapsed:Float):Void
  {
    ...
    for (i in 0...grpCapsules.activeSongItems.length)
    {
      grpCapsules.activeSongItems[i].selected = (i == curSelected);
    }
   ...
   * It will return the index of the closest song in the grpCapsules.activeSongItems array.
   * @param diff
   * @return Int
   */
  function findClosestDiff(characterVariations:Array<String>, diff:String):Int
  {
    ...
    for (index in 0...grpCapsules.activeSongItems.length)
    {
      var song:Null<FreeplaySongData> = grpCapsules.activeSongItems[index].freeplayData;
  ...
  function changeDiff(change:Int = 0, force:Bool = false, capsuleAnim:Bool = false):Void
  {
    ...
    if (daSong != null)
    {
      ...
      if (!capsuleAnim && change != 0) playJumpInAnimation(force);
    ...
    if (change != 0 || force)
    {
      ...
      for (songCapsule in grpCapsules.activeSongItems)
      {
