...
class LoadingState extends MusicBeatSubState
{
  ...
  function new(target:NextState, stopMusic:Bool, ?playParams:Null<PlayStateParams>)
  {
    super();
    this.target = target;
  ...
  override function create():Void
  {
    ...
    initSongsManifest().onComplete(function(lib) {
      ...
      if (playParams != null)
      {
        ...
        if (targetChart == null)
        {
          throw 'Couldn\'t retrieve chart data for song "${playParams.targetSong.songName}" at difficulty "$difficulty" and variation "$variation"';
        }
