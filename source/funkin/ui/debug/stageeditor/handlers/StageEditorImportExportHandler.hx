...
class StageEditorImportExportHandler
{
   ...
   * @return The file path to the latest stage backup, or null if no backups exist.
   */
  public static function getLatestBackupPath():Null<String>
  {
    ...
    // Filter to only the backups for the stage editor
    files = files.filter((file:String) -> {
      return file.endsWith(Constants.EXT_STAGE);
    });
