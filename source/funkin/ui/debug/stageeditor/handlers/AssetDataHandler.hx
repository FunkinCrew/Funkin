...
class AssetDataHandler
{
  ...
  public static function toData(obj:StageEditorObject, useBitmaps:Bool = false):StageEditorObjectData
  {
    ...
    if (useBitmaps)
    {
      ...
      outputData.xmlData = obj.generateXML("");
      return outputData;
    }
    ...
    for (name => bit in state.bitmaps)
    {
      if (areTheseBitmapsEqual(bit, obj.pixels))
      {
        ...
        outputData.xmlData = obj.generateXML(outputData.assetPath);
        return outputData;
      }
    }

    outputData.assetPath = "#FFFFFF";
    trace(outputData.assetPath);
  ...
  public static function generateXML(obj:StageEditorObject, name:String)
  {
    ...
    var xml = [
      ...
      '<TextureAtlas imagePath="${name}.png" width="${obj.pixels.width}" height="${obj.pixels.height}">'
    ].join("\n");
