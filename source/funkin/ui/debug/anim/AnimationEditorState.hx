...
class AnimationEditorState extends MusicBeatState
{
  ...
  function generateOutlines(frameShit:Array<FlxFrame>):Void
  {
    ...
    for (i in frameShit)
    {
      ...
      if (swagOutlines != null) swagOutlines.drawRect(i.uv.left * i.parent.width, i.uv.top * i.parent.height, uvW, uvH, FlxColor.TRANSPARENT, lineStyle);
  ...
  function offsetControls():Void
  {
    ...
    if (FlxG.keys.justPressed.G)
    {
      ...
      if (onionSkinChar != null && onionSkinChar.visible) updateOnionSkin();
    }

    // Plays the idle animation
      if (FlxG.keys.justPressed.SPACE && offsetAnimationDropdown != null)
    {
