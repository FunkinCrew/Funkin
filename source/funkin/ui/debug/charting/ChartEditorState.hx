...
class ChartEditorState extends UIState // UIState derives from MusicBeatState
{
  ...
  function buildAdditionalUI():Void
  {
    ...
    playbarHeadLayout.playbarHead.onDrag = function(_:DragEvent) {
      if (playbarHeadDragging)
      {
        ...
        easeSongToScrollPosition((songLengthInPixels) * playbarHeadLayout.playbarHead.value / 100);
      }
    }
  ...
  function handleScrollKeybinds():Void
  {
    ...
    if (scrollAmount != 0) easeSongToScrollPosition(currentScrollEase + scrollAmount);
    else if (!audioInstTrack.isPlaying && currentScrollEase != this.scrollPositionInPixels) easeSongToScrollPosition(currentScrollEase);
  }
  ...
  function handleCursor():Void
  {
    ...
    if (shouldHandleCursor)
    {
        ...
        if (hasDraggedMouse)
        {
          ...
          else
          {
            ...
            if (FlxG.mouse.viewY < MENU_BAR_HEIGHT)
            {
              ...
              scrollPositionInPixels -= diff * 0.5;
              easeSongToScrollPosition(currentScrollEase - (diff * 0.5)); // Too fast!
            }
      ...
      else if (notePreviewScrollAreaStartPos != null)
      {
        ...
        easeSongToScrollPosition(clickedPosInPixels);
      }
      ...
      else if (dragTargetNote != null || dragTargetEvent != null)
      {
        ...
        else
        {
          ...
          if (FlxG.mouse.viewY < MENU_BAR_HEIGHT)
          {
            // Scroll up.
            var diff:Float = MENU_BAR_HEIGHT - FlxG.mouse.viewY;
            easeSongToScrollPosition(currentScrollEase - (diff * 0.5));
          }
          else if (FlxG.mouse.viewY > (playbarHeadLayout?.y ?? 0.0))
          {
            // Scroll down.
