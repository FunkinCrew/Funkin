...
class DifficultyIcons {
  ...
  public function new(diff:Array<String>, ?defaultDifficulty:Int = 1,x:Float = 0, y:Float = 0) {
    ...
    for( level in 0...difficulties.length ) {
      ...
      var diffXml:String;
      if (FileSystem.exists(Path.normalize(System.applicationDirectory+'/assets/images/custom_difficulties/'+diffJson.difficulties[level].name+".png"))) {
         diffPic = BitmapData.fromBytes(ByteArray.fromBytes(File.getBytes(Path.normalize(System.applicationDirectory+'/assets/images/custom_difficulties/'+diffJson.difficulties[level].name+".png"))));
      } else {
      ...
      if (FileSystem.exists(Path.normalize(System.applicationDirectory+'/assets/images/custom_difficulties/'+diffJson.difficulties[level].name+".xml"))) {
         diffXml = File.getContent(Path.normalize(System.applicationDirectory+'/assets/images/custom_difficulties/'+diffJson.difficulties[level].name+".xml"));
      } else {
         // fall back on base game file to avoid crashes
