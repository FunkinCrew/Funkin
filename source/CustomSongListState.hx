...
class CustomSongListState extends MusicBeatState
{
	var songs:Array<ModSongMetadata> = [];
	...
	override function create()
	{
		...
		songArray = CoolUtil.coolTextFile("mods/songs.txt"); //doesnt need to be loaded with file system

		for (song in songArray) {
			var tempArray = song.split(':');
			addSong(tempArray[0], tempArray[1]);
		}
	...
	public function addSong(songName:String, songCharacter:String)
	{
		if (songName == "") {
			...
			songs.push(new ModSongMetadata(songName, songCharacter));
	}
