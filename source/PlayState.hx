...
class PlayState extends MusicBeatState
{
	...
	{	
		...
		trace('FYI artstyle contains ${Style.lstyle}');
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'senpai':
				dialogue = CoolUtil.coolTextFile(Paths.txt('senpai/dialogue'));
			case 'roses':
				dialogue = CoolUtil.coolTextFile(Paths.txt('roses/dialogue'));
			case 'thorns':
				dialogue = CoolUtil.coolTextFile(Paths.txt('thorns/dialogue'));
		}
	...
	override public function update(elapsed:Float)
	{
		...
		iconP1.setGraphicSize(Std.int(FlxMath.lerp(150, iconP1.width / 1.2, 0.80)));
		iconP2.setGraphicSize(Std.int(FlxMath.lerp(150, iconP2.width / 1.2, 0.80)));
		...
		if (health <= 0 && !FlxG.save.data.pmode)
		{
	...
	private function popUpScore(strumtime:Float):Void
	{
		...
		if (!Style.lstyle.name.startsWith('_PIXEL_'))
		{
			...
			comboSpr.antialiasing = Style.lstyle.antialiasing;
		}
		else
		{
		...
		{
			var numScore:FlxSprite = new FlxSprite().loadGraphic(Paths.image(pixelShitPart1 + 'num' + Std.int(i) + pixelShitPart2, Style.lstyle.library));
